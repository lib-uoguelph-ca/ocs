...
class UserRegistrationForm extends Form {
	...
	function execute() {
      ...
      sendConfirmationEmail($schedConf, $user, $registrationType);
 ...
 * See http://pkp.sfu.ca/support/forum/viewtopic.php?t=2910&f=14
 **/
function sendConfirmationEmail($schedConf, $user, $registrationType) {
    
    $schedConfName = reset($schedConf->getTitle());
    $schedConfId = $schedConf->getSchedConfId();
     
    $paramArray = array(
        'registrantName' => $user->getFullName(),
        'schedConfName' => $schedConfName,
        'registrationType' => reset($registrationType->getName()) . ' - ' . money_format('%.2n', $registrationType->getCost()) . ' ' . $registrationType->getCurrencyStringShort(),
    ...
    if ( $mail and $mail->isEnabled() ){
      $mail->setFrom(Config::getVar('email', 'default_envelope_sender'), $schedConf->getSetting('contactName', true));
